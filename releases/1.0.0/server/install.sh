...
check_existing_installation() {
    ...
    [[ -f "${INSTALL_DIR}/.env" ]] && env_exists=true
    [[ -f "${COMPOSE_FILE}" ]] && compose_exists=true
    [[ -d "${STORAGE_DIR}" ]] && storage_exists=true

    # Completely empty installation directory.
    if [[ "${env_exists}" == "false" &&
          "${compose_exists}" == "false" &&
          "${storage_exists}" == "false" ]]; then
    ...
    if ! test_valid_env; then
        echo
        echo "========================================"
        echo " Incomplete Installation Detected"
        echo "========================================"
        echo
        echo "The previous installation did not"
        echo "finish configuration."
        echo
        echo "The installer will continue from the"
